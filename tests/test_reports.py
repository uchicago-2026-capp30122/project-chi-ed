...
def test_imputer():
    ...
    assert schools.data[["free_meal_days", "crisis"]].isnull().sum().sum() == 0, (
        "There should be no missing values after imputation"
    )
...
def test_balance_panel():
    ...
    assert len(schools.data) == expected_len, (
        f"The balance panel should have {expected_len} rows"
    )
...
def test_columns_renaming():
    ...
    assert (
        "school" in schools.data.columns and "school_name" not in schools.data.columns
    ), "school_name column should be renamed to school"
    assert "date" not in schools.data.columns, (
        "Renaming method should be flexible and ignore non-existent columns"
    )
...
def test_empty_pdf():
    ...
    assert isinstance(empty_pdf_doc.sections, dict), (
        "The sections dictionary should be a dictionary"
    )
    assert len(empty_pdf_doc.sections) == 0, (
        "The sections dictionary should be empty at initialization"
    )
...
pdf_doc.add_section(
    section="Analysis",
    paragraph="analysis is an analysis of a non-analyzed content",
    figure=None,
    table=None,
)
pdf_doc.add_section(
    section="Conclusion", paragraph="Trust the process", figure=None, table=None
)
pdf_doc.add_section(
    section="References", paragraph="UChicago 2026", figure=None, table=None
)
pdf_doc.add_section(
    section="Appendix", paragraph="It's been a good ride!!!", figure=None, table=None
)
...
def test_section_access():
    ...
    assert pdf_doc.section("Conclusion")["paragraph"] == "Trust the process", (
        "The Conclusion section should have the correct paragraph"
    )
    assert pdf_doc.section("References")["paragraph"] == "UChicago 2026", (
        "The References section should have the correct paragraph"
    )
    assert pdf_doc.section("Appendix")["figure"] is None, (
        "The Appendix section should have no figure, when none added"
    )
    assert pdf_doc.section("Appendix")["table"] is None, (
        "The Appendix section should have no table, when none added"
    )
