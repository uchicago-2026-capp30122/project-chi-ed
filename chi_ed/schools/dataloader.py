...
def load_reports_card(filepath: pathlib.Path):
    ...
    other_sheets = []
    if scores_sheet:
        other_sheets.append(Schools(pandas.read_excel(excel_file, sheet_name = scores_sheet)))
    if scores2_sheet:
        other_sheets.append(Schools(pandas.read_excel(excel_file, sheet_name = scores2_sheet)))
    ...
    for sheet_dt in other_sheets:
        merged_df = pandas.merge(merged_df, sheet_dt.data, on = common_colnames, how = 'left')
