...
def plot_bar_graph(df: pandas.DataFrame, variables: dict[str, str], school1: str, school2: str,
    ...
    """Plot a grid of bar plots (nrows x ncols), one for each variable, highlighting the values for school1 and school2."""
    dataset = df.copy()
    if year is not None: dataset = dataset[dataset["year"] == year]

    # Store the individual charts for each variable, and variables with missing data to highlight
    charts = []
    ...
    def assign_highlight(scool_name: str, s1: str, s2: str, s1_label: str, s2_label: str, other_label: str) -> str:
        """Assign a highlight color to the school based on the school name"""
        if scool_name == s1:
            return s1_label
        elif scool_name == s2:
            return s2_label
    ...
    for variable, varname in variables.items():
        ...
        # We will use * to highlight the school name in the legend if it has missing data
        ...
        data["highlight"] = data["school_name"].apply(assign_highlight, 
            s1 = school1, s2 = school2, s1_label = school1_label, s2_label = school2_label, other_label = other_label
        )

        # Assign highlight color
        color_scale = alt.Scale(domain = [school1_label, school2_label, other_label],
        ...
        chart = (
            ...
            .encode(
                ...
                color = alt.Color(
                    ...
                    scale = color_scale,
                    legend = alt.Legend(
                        title = None, orient = "top-left", labelFontSize = 13, labelLimit = 400
                    ),
                ),
            )
    ...
    if school1_missing_vars:
        subtitles.append(
            f"* {school1} has missing values for {', '.join(school1_missing_vars)}"
        )
    if school2_missing_vars:
        subtitles.append(
            f"* {school2} has missing values for {', '.join(school2_missing_vars)}"
        )
...
def plot_time_series(df: pandas.DataFrame, variables: dict[str, str], school1: str, school2: str, filepath: pathlib.Path = None):
    """Plot a 2x2 grid of line plots showing school1 and school2 trends from 2019 to 2025."""
    # NOTE: This is almost a replica of the plot_bar_graph function so I am not commenting extensively
    dataset = df.copy()
    ...
    grid = alt.vconcat(
        alt.hconcat(charts[0], charts[1]), alt.hconcat(charts[2], charts[3])
    )
    ...
    if filepath is not None:
        ...
        raise Warning(
            "No filepath provided. Remember to provide a filepath to save the plot."
        )
